...
Version History:
    ...
    v5.6.1 - fixed bug where the landing rate zero time in the .csv output
        doesnt start at zero. It starts at zero now
    
    v5.7 
        Updated the circle finding algorithm to find particles at the new FOV
        Changed from f1000 to f400 lens for the final image. This reduced the
        magnification and gave circles that were about 8px in diameter

        !!!!!!!!!!!      THIS IS AN IMPORTANT CHANGE     !!!!!!!!!!!!!
        !!!!!!!!!!!              May 13 2022             !!!!!!!!!!!!!
        !!!!!!!!!!!                                      !!!!!!!!!!!!!
        !!!!!!!!!!!       Final Imaing Lens changed      !!!!!!!!!!!!!
        !!!!!!!!!!!         from ~f1000 to f400          !!!!!!!!!!!!!
        
    v6.0
        This update is to optimize the circle finding algorithm for the new
        z-stage. not much should have changed, but since there was a
        realignment and some optics were moved I thought it would be best to 
        give the script a quick optimization update.
        
        Also, I skipped a few version numbers in this script as well as in 
        the labview VI so that the back end script and labview front end share
        version numbers. 
            e.g. particle finding script v6.0 is meant to process
                data generated from iscat labview software v6.0
...
def generate_landing_rate_csv(particle_list, nframes, fps, basepath, filename):
    ...
    for i, v in enumerate(ppf):
        if v >= 1:
            ...
            break
    spf2 = spf[zeroindex:] - spf[zeroindex]
    ppf2 = ppf[zeroindex:]
...
#import random
def random_sampling(particle_list, nsamples, basepath, name, pimage_dim):
    #plots particle fit for a random selection of found particles
    ...
    for i in sampleIDs:
        ...
        save_3d_contour(darkest_image, fitG, rmseG, p.pID, "Gaussian", basepath, name, pimage_dim)
        save_3d_contour(darkest_image, fitLoG, rmseLoG, p.pID, "Laplacian of Gaussian", basepath, name, pimage_dim)
        save_3d_contour(darkest_image, fitDoG, rmseDoG, p.pID, "Difference of Gaussian", basepath, name, pimage_dim)
...
def ratiometric_particle_finder2(images, ratiometric_constants, particle_finding_constants, initial_false_positive_constants):
    
    bufsize    = ratiometric_constants['bufsize']
    clipmin    = ratiometric_constants['clipmin']
    clipmax    = ratiometric_constants['clipmax']
    pimage_dim = ratiometric_constants['pimage_dim']
    ...
    for f in tqdm.tqdm(range(n-2*bufsize)):
        ...
        frame_particles = find_particles_in_frame(i8, i16, pimage_dim, particle_finding_constants)
        ...
        for p in range(len(frame_particles)):
            ...
            if not video_particle_list:
                ...
                matchID = look_for_matching_particle(frame_particles[p], video_particle_list, initial_false_positive_constants)
...
#step 1.1 - particcle finding helper functions
    
def find_particles_in_frame(image8, image16, pimage_dim, particle_finding_constants):  #find particles in a single frame
    '''
    ...
    CANNY_SIGMA            = particle_finding_constants['CANNY_SIGMA']#1#1#3#4.5 #3   
    CANNY_LOW_THRESHOLD    = particle_finding_constants['CANNY_LOW_THRESHOLD']#100#11#10#9
    CANNY_HIGH_THRESHOLD   = particle_finding_constants['CANNY_HIGH_THRESHOLD']#200#21#19    
    HOUGH_RADII            = particle_finding_constants['HOUGH_RADII']#[3]#[8]  #8 works really well and its faster than radii ranges by around a factor of 2 or more usually
    ...
    HOUGH_THRESH           = particle_finding_constants['HOUGH_THRESH']#.3
    MIN_DIST               = particle_finding_constants['MIN_DIST']#4             # this is them minimum to distance required for two particles to be considered real (i.e. not overlapping)
    EDGE                   = particle_finding_constants['EDGE']#pimage_dim
    ...
    accums_, cx_, cy_, radii_ = hough_circle_peaks(hspaces = hough_res,             # Generate particle spec lists
                                               radii   = HOUGH_RADII,
                                               threshold = HOUGH_THRESH,                 # the lower this is the slower it goes but the more circles it finds. 0.3 is the best tradeoff 
                                               total_num_peaks = 30)
        
    
    # PRINT OUT INITIAL CIRCLES ON THIS FRAME
    # rgb_circ_image = color.gray2rgb(image8).astype(np.uint8)
    # for i in range(len(accums_)):
    #     circy, circx = circle_perimeter(r = cy_[i],  c = cx_[i],  radius = HOUGH_RADII[0],  shape = [fx,fy])
    #     rgb_circ_image[circy, circx] = (220, 20, 220)
    # print(rgb_circ_image)
    # plt.imshow(rgb_circ_image)
    # plt.show()
    
    #remove particles on edge
    cx, cy = [], []    
    for i in range(len(cx_)):
        if EDGE < cx_[i] < (fx-EDGE) and EDGE < cy_[i] < (fy-EDGE):
            cx.append(cx_[i])
            cy.append(cy_[i])
            #print(cx[-1])
    cx_, cy_ = cx, cy
    ...
    # Also record the average value of the center pixels
    # and record the particle image
    for c in range(len(cx)):
        px, py = cx[c], cy[c]
        avgdrkpxl = np.average(image16[ (py-1):(py+1), (px-1):(px+1) ])           # Calculate approximately how dark the particle is
        #print(px, py, avgdrkpxl)
        if avgdrkpxl < 1:#0.98:                                                     # If its sufficiently dark, Create a new
            #it would be nice to fix this so it pastes the iamge on a grey background
            pimage = image16[ (py-EDGE):(py+EDGE), (px-EDGE):(px+EDGE) ]                  # Particle Object and add it to the output list
    ...
    # plt.show()
    # plt.imsave(r'd:/image.png', (hough_res[0]*255))
    # plt.show()
    # #draw circles on frame and print
    # rgb_circ_image = color.gray2rgb(image8).astype(np.uint8)
    # for p in frame_particle_list:
    #     circy, circx = circle_perimeter(r = p.y_vec[0],  c = p.x_vec[0],  radius = HOUGH_RADII[0],  shape = [fx,fy])
    #     rgb_circ_image[circy, circx] = (220, 20, 220)
    # print(rgb_circ_image)
    # plt.imshow(rgb_circ_image)
    # plt.show()
...
def look_for_matching_particle(particle, particle_list, initial_false_positive_constants):

    spatial_tolerance      = initial_false_positive_constants['spatial_tolerance']#4          # pixels
    temporal_tolerance     = initial_false_positive_constants['temporal_tolerance']#2           # if a particle was found here within the last x number of frames, consider it to be the same particle
    plist_length_tolerance = initial_false_positive_constants['plist_length_tolerance']#500         # how far back do you want to search for a match in the particle list?

    matched_pID            = False       # By default, the Particle is not found on the list
    #spatial_tolerance      = 4          # pixels
    #temporal_tolerance     = 2           # if a particle was found here within the last x number of frames, consider it to be the same particle
...
# STEP 2 - Remove Short-Lived "Particles"    

def remove_blip_particles(particle_list, constants):
    st = time.time()
    ...
    for p in particle_list:
        ...
        if len(p.f_vec) > constants["Minimum Num Frames"]:
...
# Step 3 - Remove Non-PSF like "Particles"
...
def fitgaussian(data, pimage_dim): #find optimized gaussian fit for a particle
...
#%%
   # mode = "400mm lens"
    ...
    # if mode == "400mm lens":
    #     CANNY_SIGMA            = 1#1#3#4.5 #3   
    #     CANNY_LOW_THRESHOLD    = 100#11#10#9
    #     CANNY_HIGH_THRESHOLD   = 200#21#19    
    #     HOUGH_RADII            = [3]#[8]  #8 works really well and its faster than radii ranges by around a factor of 2 or more usually
    #     # more hough radii comments
    #     #[4,5,6,7,8,9]#np.arange(6,9,1)#[8]#np.arange(6,18,3)#(6,21,1) worked well but its slow i think    8 #something from 8 -12 seems to work for 512x512 images #Can also use a range: np.arange(10, 11, 2) #default 25, 35, 2
    #     HOUGH_THRESH           = .3
    #     MIN_DIST               = 4             # this is them minimum to distance required for two particles to be considered real (i.e. not overlapping)
    #     EDGE                   = pimage_dim
        
    #     #GAUSS_SIGMA            = 3.2    # KERNAL SIZE FOR INITIAL GAUSSIAN BLUR
    #     #image8 = skimage.filters.gaussian(image8, sigma=GAUSS_SIGMA)*256 #sigma=3.0
        
    # if mode == "50nm PS 500mW laser, 1000mm collection lens":
    #     CANNY_SIGMA            = 1#3#4.5 #3   
    #     CANNY_LOW_THRESHOLD    = 50#11#10#9
    #     CANNY_HIGH_THRESHOLD   = 21#19    
    ...
    # if mode == "50nm PS 5mW laser, 1000mm collection lens":
    #     CANNY_SIGMA            = 1#3#4.5 #3   
    #     CANNY_LOW_THRESHOLD    = 50#11#10#9
...
''' INITIAL CONDITIONS   '''
binfile = r'C:/Users/user1/Desktop/test data/testdata/2022-06-30-50nm PS 2000 PDL pH7 Laser120mW/VIDEOS/2022-06-30_14-56-58_raw_12_256_68.bin'
sample_name = "new stage new script test"


''' CONSTANTS '''
output_framerate = 24  # frane rate for output video

''' CIRCLE FINDING CONSTANTS '''
ratiometric_constants = {
    'bufsize'                : 10,    # 1/2 the ratiometric buffer size
    'clipmin'                : 0.95,  # black point clipping of ratiometric image
    'clipmax'                : 1.05,  # white point clipping of ratiometric image
    'pimage_dim'             : 15}    # 1/2 particle image resolution
    
particle_finding_constants = {
    'CANNY_SIGMA'            : 1,#1#3#4.5 #3   
    'CANNY_LOW_THRESHOLD'    : 100,#11#10#9
    'CANNY_HIGH_THRESHOLD'   : 200,#21#19    
    'HOUGH_RADII'            : [3],#[8]  #8 works really well and its faster than radii ranges by around a factor of 2 or more usually
    ...
    'HOUGH_THRESH'           : 0.3,
    'MIN_DIST'               : 4,             # this is them minimum to distance required for two particles to be considered real (i.e. not overlapping)
    'EDGE'                   : ratiometric_constants['pimage_dim']}

initial_false_positive_constants = {    
    'spatial_tolerance'      : 4,          # pixels
    'temporal_tolerance'     : 2,           # if a particle was found here within the last x number of frames, consider it to be the same particle
    'plist_length_tolerance' : 500}         # how far back do you want to search for a match in the particle list?

''' TEMPORAL REJECTION CONSTANTS '''
blip_constants = {'Minimum Num Frames':6}

''' PSF FIT CONSTANTS '''
psf_fit_constants = {'DoG':10}
...
''' PROCESS FROM BINFILE '''
ratio_vid8, particle_list, info = ratiometric_particle_finder2(images, ratiometric_constants, particle_finding_constants, initial_false_positive_constants)
...
particle_list2 = remove_blip_particles(particle_list, blip_constants)
save_particle_video(ratio_vid8, particle_list2, output_framerate, basepath, name, "2")

particle_list3 = remove_non_gaussian_particles(particle_list2, ratiometric_constants['pimage_dim'], psf_fit_constants)
...
print("Begin Random Samping...")
random_sampling(particle_list3, 10, basepath, name, ratiometric_constants['pimage_dim'])    
plot_contrast_histogram(particle_list3, basepath, name)
...
#binfile = r'C:/Users/Matt/Desktop/ISCAT EXPERIMENTS/2022-05-19-12nm Silica 1000x PDL pH7 Laser100mW/VIDEOS/2022-05-19_17-25-31_raw_24_512_98.bin' 

#sample_name = "12nm SiO2 on PDL"
...
root_dir = r'C:/Users/user1/Desktop/test data/testdata'
