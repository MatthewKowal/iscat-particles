...
def remove_blip_particles(particle_list):
    ...
    for p in particle_list:
        #print("Is Particle number: ", p.pID, " real? ", p.isReal())
        if p.isReal():
            cleaned_particle_list.append(p)
            cleaned_particle_list[-1].pID = c
            #print("new ID ", c, "  \t  (x, y) ", cleaned_particle_list[-1].x_vec[0], cleaned_particle_list[-1].y_vec[0], cleaned_particle_list[-1].f_vec)
            c += 1
...
def save_color_video(rgb_video_in, framerate, basepath, filename):
    ...
    #nimg=34
    #width, height = 1024, 1024
    n, x, y, colors = rgb_video_in.shape
    #print("\t ", n,x,y, colors)
    video_out = np.ones_like(rgb_video_in)
    ...
    for c in range(n):
        ...
        videoObject.write(rgb_video_in[c])
    videoObject.release()
...
binfile = r'C:/Users/user1/Desktop/video compression test/2022-03-07_15-57-22_raw_12_512_70.bin'


sample_name = "test test test"

#fov              = 12  # field of view in microns
#frame_size       = 512 # frame size in pixels
#input_framerate  = 87 # frame rate of video collection
...
#headtail     = os.path.split(binfile)
#basepath     = headtail[0]
